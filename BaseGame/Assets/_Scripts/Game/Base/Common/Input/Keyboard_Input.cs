...
public class Keyboard_Input : BaseInputController
{
    public override void CheckInput()
    {
        ...
        // store them internally in vert and horz so we don't have
        // to access them every time we need to relay input data out
        vert = Input.GetAxis("Vertical");
        horz = Input.GetAxis("Horizontal");

        // set up some Boolean values for up, down, left, right
        Up      = (vert > 0);
        Down    = (vert < 0);
        Left    = (horz < 0);
        Right   = (horz > 0);
